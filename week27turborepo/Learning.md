...
3. Deployed all 3 container on VM and docker hub with cd
4. In cd github vm first login docker hub, build the image using .dockerfile and than push it on docker hub and than github vm ssh in to our VM
   Pull new image stops previous image and in http image we mingrate the db and than run the new image on vm.
5. Created docker compose file which runs multiple containers in signle cmd.
6. One service migrates the db

7. With compose file i can create one cd workflow to build and push all these images and use docker compose to pull and start al these containers on vm with single cmd
